...
const Project = ({ projectId, containerId }) => {
    ...
    const renderLeftNav = (onClick, disabled) => {
        return (
            <img
                src={leftCaret}
                className="image-gallery-custom-left-nav image-gallery-icon"
                disabled={disabled}
                onClick={onClick}
                alt="Previous Slide"
            />
    ...
    const renderRightNav = (onClick, disabled) => {
        return (
            <img
                src={rightCaret}
                className="image-gallery-custom-right-nav image-gallery-icon"
                disabled={disabled}
                onClick={onClick}
                alt="Next Slide"
            />
    ...
    return (
        <>
            <Container
                ...
                colorStart={bgColors[projectId]}
                colorEnd={bgColors[projectId + 1]}
                restrictHeight={true}
            >
                <ProjectDiv>
                    <Content
                    ...
                    >
                        ...
                        >
                            <div className="imageModal">
                                <ImageGallery
                                    items={projectData[projectId - 1]['images']}
                        ...
                        >
                            <div className="projectDesc">
                                <p className="projectTitle">
                                    {projectData[projectId - 1]['name']}
                                </p>
                                <p className="projectSubtitle">
                                    {projectData[projectId - 1]['summary']}
                                </p>
                                <p className="projectBody">
                                    {projectData[projectId - 1]['description']}
                                </p>
                                <ul className="projectBody">
                                    {projectData[projectId - 1]['tech'].map(
                                        (li, idx) => (
                                            <li key={idx}>{li}</li>
                                        )
                                    )}
                                </ul>
                                {projectData[projectId - 1]['link'] && (
