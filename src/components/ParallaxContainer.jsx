...
const StyledContainer = styled(motion.div)`
    ...
    height: ${({ restrictHeight, headerHeight }) =>
        restrictHeight ? `calc(100vh - ${headerHeight}px)` : 'auto'};
    width: 100vw;
    ...
    @media only screen and (max-width: ${breakPoints.sm}px) {
        min-height: ${({ mobileRestrictHeight, headerHeight }) =>
            mobileRestrictHeight ? `calc(100vh - ${headerHeight}px)` : 'auto'};
    }
...
const ParallaxDiv = styled.div`
    ...
    p {
        ...
        &:nth-child(5) {
            font-size: min(max(10vw, 28px), 52px);
            padding-right: 150px;
        }
        &:nth-child(6) {
            font-size: min(max(8vw, 64px), 80px);
            padding-right: 280px;
        }
        &:nth-child(7) {
            font-size: min(max(7.5vw, 64px), 64px);
            padding-right: 30px;
        }
        &:nth-child(8) {
        ...
        &:nth-child(9) {
            font-size: min(max(3vw, 48px), 60px);
            padding-right: 80px;
        }
    }
...
const Container = ({
    ...
    const backgroundColor = useTransform(
        scrollY,
        [elementTop - bgOffsetStart, elementTop - bgOffsetEnd],
        [colorStart, colorEnd]
    );
    ...
    return (
        <StyledContainer
            ...
            mobileRestrictHeight={mobileRestrictHeight}
        >
            {
                <BgContainer style={{ y }}>
                    <ParallaxDiv>
